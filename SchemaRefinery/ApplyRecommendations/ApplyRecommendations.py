...
def process_recommendations(recommendations_file: str, 
	...
	for line in recommendations_lines:
		if line == ['#']:
			...
			continue
		locus, recommendation, *rest = line.split('\t')
	...
	for gid, recommendations in action_list.items():
		...
		for recommendation_type in recommendations:
			...
			if len(current_loci) > 0:
				pf.print_message(f'Processing {recommendation_type} for loci: {current_loci} in group {gid}', 'info')
				if recommendation_type == 'Drop':
					total_drop += len(current_loci)
					drop_ids.extend(current_loci)
					pf.print_message(f"The following loci will not be included in the schema: {', '.join(current_loci)}.", "info")
					...
					for locus in current_loci:
						...
						pf.print_message(f'File {fasta_file} copied to {destination_file}', "info")
						total_add += 1
					add_ids.extend(current_loci)
				elif recommendation_type == 'Join':
					total_join += len(current_loci)
					merged_locusID: str = current_loci[0]
					pf.print_message(f'The following loci FASTA files will be merged under the locus name {current_loci[0]}')
					...
					for locus in current_loci:
						...
						if locus in fasta_files:
							...
							records: Dict[str, str] = sf.fetch_fasta_dict(fasta_file, out)
							...
							for seqid, sequence in records.items():
								...
								if sequence_hash not in merged_records:
									...
									pf.print_message(f'Sequence {seqid} matches a previous sequence already added.', "info")
									continue
						else:
							pf.print_message(f'File {locus} not found in the FASTA folder', "info")
				...
				pf.print_message(f'No loci to process for recommendation {recommendation_type} in group {gid}', 'info')
	...
	pf.print_message(f'The final schema has a total of {len(os.listdir(schema_directory))} loci.', 'info')
