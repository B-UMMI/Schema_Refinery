...
def consolidate_annotations(consolidate_annotation_files: List[str],
    ...
    first_1_filtered = first_df[(first_df.iloc[:, 1] != 'Not matched')].sort_values(by=first_df.columns[1]).drop_duplicates(subset=first_df.columns[1]).replace('', np.nan).dropna(subset=first_df.columns[1]).reset_index(drop=True)
    second_1_filtered = second_df[(second_df.iloc[:, 1] != 'Not matched')].sort_values(by=second_df.columns[1]).drop_duplicates(subset=second_df.columns[1]).replace('', np.nan).dropna(subset=second_df.columns[1]).reset_index(drop=True)
    ...
    if len(consolidate_annotation_files) > 2:
        ...
        for i in range(2, len(consolidate_annotation_files)):
            ...
            if best_match == 'f0s0':
                ...
                prf.print_message(f"Ids from the consolidated file that didn't match: {len(mismatched_f0s0)}", 'info')
                mismatched_s0f0 = new_0_filtered.iloc[:, 0][~new_0_filtered.iloc[:, 0].isin(old_0_filtered.iloc[:, 0])]
                prf.print_message(f"Ids from the {i+1} file that didn't match: {len(mismatched_s0f0)}", 'info')
                merged_table = upf.merge_files_by_column_values_df(old_df,
                                                    new_0_filtered,
                                                    0,
                                                    0,
                                                    output_file,
            ...
            # Compare the first column of the first file and the second one of the second file
                ...
                prf.print_message(f"Ids from the consolidated file that didn't match: {len(mismatched_f0s1)}", 'info')
                mismatched_s1f0 = new_1_filtered.iloc[:, 1][~new_1_filtered.iloc[:, 1].isin(old_0_filtered.iloc[:, 0])]
                prf.print_message(f"Ids from the {i+1} file that didn't match: {len(mismatched_s1f0)}", 'info')
                merged_table = upf.merge_files_by_column_values_df(new_df,
                                                    old_0_filtered,
                                                    1,
                                                    0,
                                                    output_file,
            ...
            # Compare the second column of the first file and the first one of the second file
                ...
                prf.print_message(f"Ids from the consolidated file that didn't match: {len(mismatched_f1s0)}", 'info')
                mismatched_s0f1 = new_0_filtered.iloc[:, 0][~new_0_filtered.iloc[:, 0].isin(old_1_filtered.iloc[:, 1])]
                prf.print_message(f"Ids from the {i+1} file that didn't match: {len(mismatched_s0f1)}", 'info')
                merged_table = upf.merge_files_by_column_values_df(old_df,
                                                    new_0_filtered,
                                                    1,
                                                    0,
            ...
            # Compare the second columns of both files
                ...
                prf.print_message(f"Ids from the consolidated file that didn't match: {len(mismatched_f1s1)}", 'info')
                mismatched_s1f1 = new_1_filtered.iloc[:, 0][~new_1_filtered.iloc[:, 1].isin(old_1_filtered.iloc[:, 1])]
                prf.print_message(f"Ids from the {i+1} file that didn't match: {len(mismatched_s1f1)}", 'info')
                merged_table = upf.merge_files_by_column_values_df(old_df,
                                                    new_1_filtered,
                                                    1,
                                                    1,
