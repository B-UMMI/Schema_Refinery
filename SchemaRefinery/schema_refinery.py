...
def refine_schema() -> None:
	...
	parser.add_argument('-i',
						...
						required=False,
						dest='input_schemas',
						help='List of paths to schema directories.')

	parser.add_argument('-a',
						'--allelecall-directories',
						type=str,
						nargs='+',
						required=True,
						dest='allelecall_directories',
						help='Path to the directory that contains the allele calling results '
							 'generated by chewBBACA. If the `cds` run mode is selected, the'
							 ' folder must contain the `temp` folder (kept in the schema if'
							 ' the `--no-cleanup` option is used) and the `unclassified_cds.fasta`'
							 ' file (created when the `--output-unclassified` option is used).')
	...
	parser.add_argument('-sc',
						...
						type=float,
						required=False,
						dest='special_classifications',
						default=0.3,
						help='The maximum percentage of special classifications a locus can have to be evaluated. Loci with a '
							 'higher percentage of special classifications are excluded.')
	...
	parser.add_argument('-m',
						...
						help='Run mode used to evaluate the input. The module '
							 'can evaluate the loci from a single schema, accept'
							 ' multiple schemas to compare the loci from all '
							 'schemas, and cluster the CDSs in a FASTA file to'
							 ' define putative loci units that are then evaluated. '
							 'When providing multiple schemas, make sure that '
							 'different schemas do not contain loci with the same '
							 'identifiers.')
   ...
   # Delete --debug and --logger arguments because we set up a global variable both,
	...
	if args.run_mode == 'cds':
		if not os.path.exists(ff.join_paths(args.allelecall_directories[0], ["temp"])):
