...
def graph_html(networkx_graph, output_html):
    """
    """
...
input_file = '/home/rfm/Desktop/rfm/Lab_Analyses/GAS_PrepExternalSchema/wgMLST_schema/spyogenes_schema/solve_problematic_2/scl_analysis/GCF-002236855-protein1020_protein.fasta'
...
def main(input_file, output_dir):
    ...
    records = [((rec.id).split('_')[-1], str(rec.seq)) for rec in SeqIO.parse(input_file, 'fasta')]
    ...
    # get nodes and add to graph
    #nodes = list(set([l[0] for l in blast_results_filtered]))
    #G.add_nodes_from(nodes)

    edges = [(l[0], l[1]) for l in blast_results_filtered if float(l[-1]) >= 0.8]
    G.add_edges_from(edges)

    # create HTML with grpah viz
    output_html = os.path.join(output_dir, 'nx.html')
    graph_html(G, output_html)
